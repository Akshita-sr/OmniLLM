...
class TestExperimentLogger:
    ...
    def test_get_records_filter_by_session(self):
        for cond, sid in [("A", "s1"), ("B", "s2"), ("C", "s1")]:
            self.logger.log_interaction("s1" if sid == "s1" else "s2", "P001", cond, "info_retrieval", "q", "a", "m")
        records = self.logger.get_records(session_id="s1")
        assert all(r.session_id == "s1" for r in records)

    def test_get_records_filter_by_condition(self):
        for cond in ["A", "B", "A", "C"]:
            self.logger.log_interaction("s1", "P001", cond, "info_retrieval", "q", "a", "m")
        records = self.logger.get_records(condition="A")
        assert len(records) == 2
        assert all(r.condition == "A" for r in records)

    def test_get_records_filter_by_task_type(self):
        for t in ["navigation", "info_retrieval", "navigation"]:
            self.logger.log_interaction("s1", "P001", "A", t, "q", "a", "m")
    ...
    def test_get_records_filter_by_model_id(self):
        for mid in ["openai-gpt4o-mini", "claude-haiku", "openai-gpt4o-mini"]:
            self.logger.log_interaction("s1", "P001", "A", "social_conversation", "q", "a", mid)
    ...
    def test_get_summary_with_records(self):
        self.logger.log_interaction(
            "s1", "P001", "A", "info_retrieval", "q", "a", "openai-gpt4o-mini",
            latency_ms=300.0, judge_score=0.9, task_success=True,
        )
        self.logger.log_interaction(
            "s1", "P002", "B", "navigation", "q2", "a2", "llama3-8b-local",
            latency_ms=500.0, judge_score=0.7, task_success=False,
        )
        summary = self.logger.get_summary()
        assert summary["total_interactions"] == 2
        assert "openai-gpt4o-mini" in summary["by_model"]
        assert "llama3-8b-local" in summary["by_model"]
        assert "A" in summary["by_condition"]
        assert "B" in summary["by_condition"]

    def test_get_summary_avg_latency(self):
        self.logger.log_interaction("s1", "P001", "A", "info_retrieval", "q", "a", "m", latency_ms=200.0)
        self.logger.log_interaction("s1", "P001", "A", "info_retrieval", "q", "a", "m", latency_ms=400.0)
        summary = self.logger.get_summary()
        assert summary["by_model"]["m"]["avg_latency_ms"] == pytest.approx(300.0)

    def test_save_and_load(self, tmp_path: Path):
        self.logger.log_interaction(
            "s1", "P001", "C", "multilingual", "Bonjour", "Hello!", "gemini-flash",
            language="fr",
        )
    ...
    def test_save_creates_valid_json(self, tmp_path: Path):
        self.logger.log_interaction("s1", "P001", "A", "info_retrieval", "q", "a", "m")
        path = tmp_path / "records.json"
        self.logger.save(path)
    ...
    def test_save_csv(self, tmp_path: Path):
        self.logger.log_interaction("s1", "P001", "A", "navigation", "q", "a", "m")
    ...
    def test_total_cost_in_summary(self):
        self.logger.log_interaction("s1", "P001", "A", "info_retrieval", "q", "a", "m", cost_usd=0.001)
        self.logger.log_interaction("s1", "P001", "A", "info_retrieval", "q", "a", "m", cost_usd=0.002)
        summary = self.logger.get_summary()
        assert summary["total_cost_usd"] == pytest.approx(0.003)

    def test_success_rate_in_summary(self):
        self.logger.log_interaction("s1", "P001", "A", "info_retrieval", "q", "a", "m", task_success=True)
        self.logger.log_interaction("s2", "P001", "A", "info_retrieval", "q", "a", "m", task_success=False)
        summary = self.logger.get_summary()
        assert summary["by_condition"]["A"]["success_rate"] == pytest.approx(0.5)
