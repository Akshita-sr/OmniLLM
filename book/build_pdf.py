"""Build the OmniLLM book PDF from the master Markdown file.

Pipeline: Markdown -> HTML (markdown lib) -> PDF (xhtml2pdf, pure Python).

Renders the box-drawing diagrams, arrows and special symbols correctly by
registering DejaVu Sans + DejaVu Sans Mono with ReportLab before xhtml2pdf
runs. Strips out emoji that DejaVu cannot render and replaces them with
short text equivalents so nothing falls back to a tofu square.
"""
...
# ─── Font registration ────────────────────────────────────────────────────────
# DejaVu Sans + DejaVu Sans Mono ship with matplotlib. They have full
# coverage of the box-drawing characters (U+2500..U+257F), arrows
# (U+2190..U+21FF) and common geometric shapes that the original Helvetica
# fallback rendered as tofu squares.
...
def register_fonts() -> None:
    ...
    # Tell reportlab how the four variants relate to the family name used in CSS.
...
_EMOJI_REPLACE = {
    ...
    "⚡": "*",                # ⚡ HIGH VOLTAGE (DejaVu has weak glyph)
    "⚠": "(!)",              # ⚠ WARNING SIGN
    "️": "",                 # Variation Selector-16 (invisible joiner)
    "̃": "",            # Combining tilde — xhtml2pdf does not compose marks;
                             # rendered as a tofu square. Drop the diacritic so
                             # "C̃" reads as "C" (the normalisation is explained
...
# appear ONLY inside fenced code blocks in the source (verified by grep), so
# substituting globally is safe.
#
# The replacements preserve roughly the right horizontal proportion so the
# diagrams stay aligned when rendered in a monospace font.
...
def clean_markdown(text: str) -> str:
    """Strip pandoc YAML frontmatter and LaTeX commands; replace problem emoji
    and box-drawing characters that xhtml2pdf renders as squares."""
    if text.startswith("---"):
        end = text.find("\n---", 3)
        if end != -1:
...
def md_to_html(text: str) -> str:
    ...
    html = (
        ...
        "<title>OmniLLM Book</title>"
        f"<style>{CSS}\n"
        ".pageBreak { page-break-before: always; }"
        "</style></head><body>"
        + body
        + "<div id='footerContent' style='text-align:center;font-size:8pt;color:#888;'>"
          "OmniLLM &mdash; The Complete Book &nbsp;|&nbsp; <pdf:pagenumber/></div>"
        "</body></html>"
    )
    return html
...
def main() -> None:
    ...
    print(f"[build_pdf] Reading {MASTER_MD}", flush=True)
    raw = MASTER_MD.read_text(encoding="utf-8")
    cleaned = clean_markdown(raw)

    print(f"[build_pdf] Converting Markdown -> HTML ({len(cleaned)} chars)", flush=True)
    t0 = time.time()
    html = md_to_html(cleaned)
    HTML_OUT.write_text(html, encoding="utf-8")
    print(f"[build_pdf] HTML done in {time.time()-t0:.1f}s ({len(html)} chars)", flush=True)
